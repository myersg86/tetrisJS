...
var Page = {
  ...
  // calculate the unit size, canvas bounds, and canvas positioning
  WindowChanged: function() {
    
    // Calulcate the unitSize based on window width and height.
    // The minimums will be used.
    ...
    // if the calcUnitMin != unitSize, update unitSize, recalculate objects,
    // and update the canvas element bounds
    
    this.unitSize = newUnitMin;

    // store this right-most & bottom-most points for canvas bounds
    var rightLimit = 0,
        bottomLimit = 0;
  ...
  Initialize: function(){
    ...
    // if the page has not been set up, do initial setup
  ...
  //redraws canvas visual when page is marked as dirty
  Update: function() {
    for(var i = 0; i < Page.AreaArr.length; i++){
...
function DrawAreaObj(Left,Top,Width,Height,DrawFunction){
  ...
  //draw function as passed in by the caller
  this.Draw = DrawFunction;
  
  // push this area into area array
  ...
  for(var i = 0; i < GM.StaticUnits.length; i++){
    for(var j = 0; j < GM.StaticUnits[i].length; j++){
      ...
      if (uValue !== 0){
        ...
        // fill out this square with color based on player status (alive)
...
Page.UpcomingB = new DrawAreaObj(10.5,5.2,2.5,2.5,function(){
  ...
  // draw the upcoming piece (if one exits)
  if (pcB !== 0){
    ...
    // draw the upcoming piece, using avg vars to center
